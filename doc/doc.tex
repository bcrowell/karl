...
\begin{equation*}
  x = \sqrt{|r-1|}\exp\left(\pm\frac{r+t}{2}\right),
\end{equation*}
where the $+$ sign holds for the computation of $a$ in region I or $b$ in region II. Then
\begin{align*}
  a &= \sinh^{-1} x \\
  b &= -\sinh^{-1} x' \quad \text{[other value of x]} \\
\end{align*}
If $x$ is too big to be represented in floating point, then we have the approximation
$\sinh^{-1} x = \ln(2x)+O(x^{-2})$. It can happen close to the horizon that the exponential
