...
For a null geodesic with $L\ne0$, it looks from a numerical example
like maybe $r\propto \lambda^{1/2}$, but it's
hard to be sure about the exponent with this technique, possibly $r$ behaves
worse than any power near $\lambda=0$, maybe like a log or something.
A quick and sloppy analysis of $\tau(r)$ in the limit $E\rightarrow\infty$
gives $\propto \tau^{2/5}$, i.e., the same as for $m\ne0$, since the same term in
the integrand for $\tau{r}$ still dominates.
...
If $r>r_\text{min}$, take the step size to be
\begin{equation*}
  \Delta\lambda=k\delta^{1/4}(\lambda_s-\lambda)^\alpha
\end{equation*}
(but never greater than $\lambda_s-\lambda$).
...
The adjustable parameters are $k$, $\alpha$, and $r_\text{min}$. Tune these up so that
the desired precision $\delta$ is achieved with minimum computational effort.
From my previous empirical studies, I think $\alpha\approx 1$, $k\approx 10$,
$r_\text{min}\approx 10^{-8}$.
