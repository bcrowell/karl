...
\end{equation*}
where $a=L^{-2}$ and $b=L^{-2}(E^2-1)$. I think $b$ can have either sign.
...
Try an empirical rule with three parameters:
$\Delta\lambda=k(\lambda_s-\lambda)^{-\alpha}$, starting when $\epsilon>0$
and ending at $\lambda_s-\lambda=\lambda_\text{min}$. Goal is to get
to 16-digit precision with minimum computation.  For massive particles
in the non-ultrarelativistic regime ($E$ not $\gg 1$), it seems
reasonable to hope that there would be some universal value of $k$. 
