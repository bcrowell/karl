...
Define a variable $\lambert $ whose magnitude is $|\lambert |=|r-1|$
and whose sign is defined later.
\begin{align}
...
$\rho=0$, both singularities at $\rho=-1$. Also, let
\begin{equation}
  \lambert  = W(\rho/e),
\end{equation}
which is consistent with the earlier definition of $|\lambert |=|r-1|$ and determines its sign as well.
...
where
\begin{equation}
  B = \frac{4\lambert }{(1+\lambert )\rho} = \frac{4}{r}e^{-r}.
\end{equation}
Here $(1+1/\lambert )\rho=re^r$ by the identity $e^{W(x)}=x/W(x)$, and $1+\lambert =r$. 
...
\begin{align}
\Gamma\indices{^V_V_V} &= -W\rho^{-1}r^{-2}-V^{-1}  \\
\Gamma\indices{^W_W_W} &= -V\rho^{-1}r^{-2}-W^{-1}  \\
\Gamma\indices{^\theta_V_\theta} = \Gamma\indices{^\phi_V_\phi} &= -Wr^{-2}e^{-r}  \\
\Gamma\indices{^\theta_W_\theta} = \Gamma\indices{^\phi_W_\phi} &= -Vr^{-2}e^{-r}  \\
%
\Gamma\indices{^V_\theta_\theta} &= -\frac{1}{2}B e^r V \\
\Gamma\indices{^W_\theta_\theta} &= -\frac{1}{2}B e^r W \\
\Gamma\indices{^V_\phi_\phi} &= -\frac{1}{2}B e^r V \sin^2\theta\\
\Gamma\indices{^W_\phi_\phi} &= -\frac{1}{2}B e^r W \sin^2\theta\\
%
