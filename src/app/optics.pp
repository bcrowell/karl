...
def make_star_table(star_catalog,aberration_table,v_table,r,if_black_hole,if_fake,ra_out,dec_out,max_mag,\
  ...
  [r,alpha,beta,beta-alpha,dbeta/dalpha]. Here alpha is the observer's angle.
...
def real_stars(table,aberration_table,r,if_black_hole,ra_out,dec_out,max_mag,star_catalog,m,m_inv,v_table):
  ...
  for i in range(n_stars):
    ...
    if if_black_hole:
      dbeta_dalpha = math_util.linear_interp_from_table(aberration_table,2,4,beta)
      f = amplification(alpha,beta,dbeta_dalpha)
    else:
      f=1.0
...
def fake_stars(table,aberration_table,r,if_black_hole,ra_out,dec_out,max_mag,m,m_inv,star_catalog_max_mag,\
  ...
  for i in range(len(aberration_table)-1):
    ...
    for j in range(n_az):
      ...
      f_approx = amplification(ab1[1],ab1[2],ab1[4])
      ...
      if star_catalog_max_mag<max_mag+mag_corr:
        ...
        for i in range(k):
          ...
          for j in range(poisson_random(dn)):
            ...
            brightness = brightness_helper(alpha,beta,mag,ab1[4],ab2[4],beta1,beta2,if_black_hole)
...
def amplification(alpha,beta,dbeta_dalpha):
  if dbeta_dalpha==0.0 or beta==0.0:
    return 1/EPS
  f = abs((1/dbeta_dalpha)*sin(alpha)/sin(beta))
  if f<EPS: # can have f<0 due to interpolation
    f=EPS
  return f

def brightness_helper(alpha,beta,mag,dbeta_dalpha1,dbeta_dalpha2,beta1,beta2,if_black_hole):
  if if_black_hole:
    dbeta_dalpha = math_util.linear_interp(beta1,beta2,dbeta_dalpha1,dbeta_dalpha2,beta)
    f = amplification(alpha,beta,dbeta_dalpha)
  else:
    f=1.0
  brightness = exp(-(mag/2.5)*log(10.0)+log(f))
