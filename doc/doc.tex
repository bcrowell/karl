...
The following describes my results as of 2018 August 20.

One of my goals was to accurately find the proper time for test particles to hit the singularity,
with and without an external force, so as to be able to reproduce Lewis and Kwan's results
and possibly extend them. I basically got this working, and have some tests in the test
suite. I later fiddled with the adaptive step size, and altered the algorithms for detecting
when we were going to hit the singularity in ways that could affect the precision of the
proper time. As of August 2018, I
need to go back and test this more thoroughly and extend the test suite before
trusting the code enough to really use it for high-quality work on this topic.

For optics in the Schwarzschild spacetime, I developed three different
algorithms: (1) the same adaptive algorithm
I'd previously developed, (2) a different adaptive algorithm for use with AKS,
and (3) reducing $\phi(r)$ to quadrature. Method 3 is very fast and reliable at all r,
but cannot be extended to other spacetimes. 
There is also a method (4), which is that $r(\phi)$ can be expressed
in closed form in terms of the Weierstrass elliptic function $\wp$ (Gibbons and Vyska).
For the Weierstrass method, Gibbons expresses the setup
in terms of a certain integral; this integral is actually the inverse of the Weierstrass
function, which can be evaluated efficiently (see
\url{http://arblib.org/acb_elliptic.html#weierstrass-elliptic-functions}).

For null geodesics in the Schwarzschild spacetime, there are excellent approximations for
small and large deflections (see notes elsewhere). I think it could be a fun project to
put these together into some kind of approximation scheme. Although there are exact results
in terms of $\wp$, they're not typically very practical, for the reasons given above.
...
get all interesting visible effects. Small deflections are basically
SR doppler shifts, because the observer is free-falling.
...
If that doesn't work, then it restarts the whole calculation over again from scratch with a smaller step
size.
Step size is chosen to be $\propto r^2$ according to the initial $r$ of each block. This makes it very
slow for small $r$, but seems necessary --- $\propto r$ wasn't enough.
...
work. However, as I get below about r=0.15 I can't seem to get this approach to work. For the smallest values
of $r$, I've had to resort to using the fact that $\phi(r)$ can be expressed as an indefinite integral, but
this approach won't work for other spacetimes.

Another possible avenue might be to fiddle with other coordinate systems in which the geodesic
equation might be better behaved. For example, D'Ambrosio and Rovelli (1803.05015) work with the coordinate
$\tau=r^{1/2}$, and get nice well-behaved Taylor series (eq. 32) for $t$ and $\phi$ in terms of
$\tau$. (Confusingly,
they notate Schwarzschild $t$ as $x$.)

\subsection{Approximations to the deflection angle}

See subdirectory approx.

For small $\alpha$, the slope of the graph appears to be exactly
\begin{equation*}
  \left.\frac{d\beta}{d\alpha}\right|_0 = 1-(1+\sqrt{r})^{-1}.
\end{equation*}
See slope.csv. This appears to work to within the numerical accuracy of the Runge-Kutta for all
values of $r$ inside and outside the horizon. At large $r$, this is simply the semi-Newtonian
aberration of $r^{-1/2}$.

Behavior of deflection at grazing: For angles near $\alpha_\text{max}$, it seems empirically
that $k=\beta+\ln(\alpha_\text{max}-\alpha)+\ln r$ is a slowly
varying function of $\alpha$ and of $\ln r$, and approaches a constant as
$r\rightarrow\infty$. There is a close physical analogy
with a pendulum at large amplitudes, for which the period varies as an elliptic integral,
whose asymptotic behavior is $K(1-\epsilon)\approx(1/2)\ln(2\epsilon)$.
It seems fortuitous that the coefficients on the second and third terms are
exactly 1.
This is hard to verify at large $\beta$, since calculations may be inaccurate, motion is unstable.
At large $r$, $k\approx 3.70$. For $0.16\le r\le\infty$, it looks like $k$ varies by about
6 radians. 

Bozza (gr-qc/0102068) has a treatment of the large $r$ limit that clarifies a lot of this.
They get an approximation which I think in my notation is
$\beta-\alpha\approx -(L/E-L/E0)+A$, where $A=-\ln[(5+3\sqrt3)/1944]-\pi$.
This seems to amount to a proof that the coefficients on the log terms are both 1.
I think this implies that the limiting value of $k$ should be $\pi+A=5.250$, which
is wrong compared to my value of $3.70$. 

This is good enough to tame the logarithmic divergence
in the function $\beta(\alpha)$ and allow excellent interpolation and extrapolation.
I spent some time trying to improve on this approximation but didn't come up with anything super
nice.

I tried replacing $\ln r$ with $\ln(r+f(r))$, which at large $r$ is approximately
like $\ln r\rightarrow \ln r + f/r$. I can simply take the data and solve for $f$. The
resulting function $f(r)$ is positive and concave up, appears only vaguely parabolic when graphed
against $\ln r$, can't see any nice limiting behavior at small or large values of $\ln r$.

At small $r$, I thought there might be some trig going on, but in fact the error in the
approximation at small $r$ is rather large, much too large to be accounted for by introducing
any reasonable trig function like $\cos\alpha_\text{max}$.

This kind of thing is discussed in Bozza,
``Strong field limit of black hole gravitational lensing,''
\url{https://arxiv.org/abs/gr-qc/0102068}
...
Two open-source libraries by Riccardo Antonelli: 
\url{https://github.com/rantonels/starless} \url{https://github.com/rantonels/schwarzschild}

some guy's web page with nice images, black hole ray tracing:
\url{http://www.stephenbrooks.org/misc/blackhole/}

Perlick, ``Gravitational Lensing from a Spacetime Perspective,''
\url{https://arxiv.org/abs/1010.3416} -- long review article

Marck, ``Shortcut method of solution of geodesic equations for
Schwarzschild black hole,'' \url{https://arxiv.org/abs/gr-qc/9505010}

Bozza, ``Gravitational Lensing by Black Holes,''
\url{https://arxiv.org/abs/0911.2187}
...
D'Ambrosio and Rovelli, ``How Information Crosses Schwarzschild's Central Singularity,''
\url{http://arxiv.org/abs/1803.05015} -- interesting qg paper, gives Taylor expansion
for null geodesics near the singularity

Gibbons and Vyska, ``The Application of Weierstrass elliptic functions to
Schwarzschild Null Geodesics,'' \url{http://arxiv.org/abs/1110.6508} --
Expresses $r(\phi)$ using Weierstrass elliptic function $\wp$.
