...
I'm confused about the case of emission from outside the horizon and
detection from inside.  The following papers talk about this sort of
...
the universe. But none of them seem to answer the seemingly reasonable
question of what happens for emission from infinity and observation from inside
the horizon, say for an observer infalling from rest at infinity. In this case,
I imagine that the observer falls along a trajectory such that the energy
$Au^t$ is a constant, and the ray also has constant $Av^t$. Then the result
I get is simply $\omega_\text{obs}/\omega_\text{em}=A^{-1}$, where $A$ is evaluated
at the observer's location. But this doesn't seem to make much sense, because it
blows up at the horizon and is negative inside the horizon. They all address
these issues by leaping directly to kruskal coordinates, but I would feel more
satisfied with an answer that didn't appeal to coordinates. Is it true, or is it false,
that the conserved energy is conserved within the horizon?
