...
# For testing purposes, return the metric in KS coordinates using the expressions
# (translated into python) that were input into maxima to get the Christoffel 
# symbols (chrostoffel.mac).
def sch_metric_ks_maxima_input(v,w,theta):
  g = [[0 for i in range(4)] for j in range(4)]
  x = (v-w)/2 
  t = (v+w)/2 
  rho = x**2-t**2
  ell = lambert_w(rho/math.e)
  b = 4*ell/((1+ell)*rho)  
  g[0][1] = b/2
  g[1][0] = b/2
  g[2][2] = -(1+ell)**2
  g[3][3] = -(1+ell)**2*sin(theta)**2
  return g
...
def sch_christoffel_ks_raw_maxima(v,w,theta,r):
  ...
  #   ... ^phi _phi w
  ch[2][2][0] = (exp((-(r-1))-1)*v**2*w)/(2*(r-1))+(exp((-(r-1))-1)*v**2*w)/2 
  #   ... ^v _theta theta
  ch[2][2][1] = (exp((-(r-1))-1)*v*w**2)/(2*(r-1))+(exp((-(r-1))-1)*v*w**2)/2 
  #   ... ^w _theta theta
  ch[2][3][3] = cos(theta)/sin(theta) 
  #   ... ^phi _theta phi
  ch[3][2][3] = cos(theta)/sin(theta) 
  #   ... ^phi _phi theta
  ch[3][3][0] = (exp((-(r-1))-1)*sin(theta)**2*v**2*w)/(2*(r-1))+(exp((-(r-1))-1)*sin(theta)**2*v**2*w)/2 
  #   ... ^v _phi phi
  ch[3][3][1] = (exp((-(r-1))-1)*sin(theta)**2*v*w**2)/(2*(r-1))+(exp((-(r-1))-1)*sin(theta)**2*v*w**2)/2 
  #   ... ^w _phi phi
