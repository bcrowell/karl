...
For the Christoffel symbols in the arcsinh-Kruskal coordinates, I
ran maxima using a form of the metric that is
easy to calculate without  overflows in region I. The nearly
raw results are:
\begin{align*}
\Gamma\indices{^a_a_a} &= (\sinh(a)PQ^2+(2\sinh(a)P-e^a \cosh(a))Q+\sinh(a)P-2e^a \cosh(a))/\\
                       & \qquad (\cosh(a)P+2\cosh(a)PQ+\cosh(a)PQ^2)  \\
\Gamma\indices{^i_a_i} &= e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^j_a_j} &= e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^k_a_k} &= e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^b_b_b} &= (P\sinh(b)Q^2+(e^a \cosh(b)+2P\sinh(b))Q+P\sinh(b)+2e^a \cosh(b))/\\
                       & \qquad (P\cosh(b)+2P\cosh(b)Q+P\cosh(b)Q^2)  \\
\Gamma\indices{^i_b_i} &= -e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^j_b_j} &= -e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^k_b_k} &= -e^a /(P+2PQ+PQ^2)  \\
\Gamma\indices{^a_i_i} &= -(e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  \\
\Gamma\indices{^b_i_i} &= (e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  \\
\Gamma\indices{^a_j_j} &= -(e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  \\
\Gamma\indices{^b_j_j} &= (e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  \\
\Gamma\indices{^a_k_k} &= -(e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  \\
\Gamma\indices{^b_k_k} &= (e^{-b}(e^a Q+e^a ))/(2\cosh(a)\cosh(b))  
\end{align*}
Here $Q=W(e^{a-b-1})$ and $P=e^{Q+b+1}$.

In addition, let $S=e^{a-b-1-Q}(1+Q)^{-2}$ and
$T=e^{a-b}(Q+1)/(2\cosh a\cosh b)$. Then the above expressions can be written in the following
forms, which can be evaluated without overflows:
\begin{align*}
\Gamma\indices{^a_a_a} &= \tanh a - (Q+2)S  \\
\Gamma\indices{^b_b_b} &= \tanh b + (Q+2)S  \\
\Gamma\indices{^i_a_i} &= S \\
\Gamma\indices{^a_i_i} &= -T  \\
\end{align*}
...
This requires that $S$ and $T$ flip signs while $Q$ stays the same. But in fact these symmetries
do not seem to hold. I'm not sure why. For computational convenience, I always pre-flip everything
to I or II, so it's possible that it won't matter. No, fails tests, this is wrong.

Retry, using simpler form of metric:
\begin{align*}
